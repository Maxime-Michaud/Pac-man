...
void dragonShout::draw(sf::RenderTarget & rt, sf::RenderStates rs) const
{
	...
	float temps = _clockDragonShout.getElapsedTime().asMilliseconds() - 700;
	float ratio = _clockDragonShout.getElapsedTime() / sf::milliseconds(3000);
	if (ratio > 1)
		ratio = 1;
	for (int i = 1; i <= 30; i++)	//Le premier cercle
	{
		pos.x = temps * cos(2 * (float)M_PI * (i - 1) / 30) + _pos.x;
		pos.y = temps * sin(2 * (float)M_PI * (i - 1) / 30) + _pos.y;
		...
		pos.x = temps * cos(2 * (float)M_PI * i / 30) + _pos.x;
		pos.y = temps * sin(2 * (float)M_PI * i / 30) + _pos.y;
	...
	{
		pos.x = temps / 3 * cos(2 * (float)M_PI * (i - 1) / 30) + _pos.x;
		pos.y = temps / 3 * sin(2 * (float)M_PI * (i - 1) / 30) + _pos.y;
		...
		//Position du nouveau vertex
		pos.x = temps / 3 * cos(2 * (float)M_PI * i / 30) + _pos.x;
		pos.y = temps / 3 * sin(2 * (float)M_PI * i / 30) + _pos.y;
	...
	{
		pos.x = temps * 2 * cos(2 * (float)M_PI * (i - 1) / 30) + _pos.x;
		pos.y = temps * 2 * sin(2 * (float)M_PI * (i - 1) / 30) + _pos.y;
		...
		//Position du nouveau vertex
		pos.x = temps * 2 * cos(2 * (float)M_PI * i / 30) + _pos.x;
		pos.y = temps * 2 * sin(2 * (float)M_PI * i / 30) + _pos.y;
